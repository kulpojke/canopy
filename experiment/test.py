...
training_model = tree_classifier.get_patch_model((input_shape[0], input_shape[1], input_shape[2] + 1))
training_model.load_weights(args.out + '/' + weights_uri)
model = tree_classifier.get_convolutional_model((input_shape[0], input_shape[1], input_shape[2] + 1))
​
​
print('model summary is that of:')
print(model.summary())
​
...
​
print('predict is that of ***********')
print(args.out + '/' + predict_uri)
​
​
# open the CHM
...
​
​
            chm_tile = chm_tile[..., np.newaxis]
            image_tile = np.concatenate((image_tile, chm_tile), axis=-1)
            image_tile = np.stack(image_tile, axis=0)
            # run tile through network
            #prediction = model.predict(np.array(image_tile)).astype('uint8')
