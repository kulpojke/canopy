...
​
print('x_shape and x_dtype')
print(x_shape, x_dtype)
print('y_shape and y_dtype')
print(y_shape, y_dtype)
​
classifier = PatchClassifier(num_classes=8)
# note (15, 15, 33) is simply x_shape with an additional channel
model = classifier.get_patch_model((15, 15, 33))
​
print('model summary: ')
print(model.summary())
​
...
for i in range(0, len(x_all)):
    ...
    h = h[..., np.newaxis]
    #h = h - np.mean(h)
    final = np.concatenate((x, h), axis=-1)
    xh_all.append(final)
​
x_all = np.stack(xh_all, axis=0)
#x_all = xh_all
​
