...
patch_radius = 7
height_threshold = 5
train_image_patches, train_patch_labels, train_patch_heights = extract_patches(my_image_uri,patch_radius,chm_uri,height_threshold,args.out + '/' + train_labels_uri)
val_image_patches, val_patch_labels, val_patch_heights = extract_patches(my_image_uri,patch_radius,chm_uri,height_threshold,args.out + '/' + val_labels_uri)
test_image_patches, test_patch_labels, test_patch_heights = extract_patches(my_image_uri,patch_radius,chm_uri,height_threshold,args.out + '/' + test_labels_uri)
​
...
with h5.File(args.out + '/' + train_data_uri,'w') as f:
  ...
  f.create_dataset('height', data=train_patch_heights)
with h5.File(args.out + '/' + val_data_uri,'w') as f:
  ...
  f.create_dataset('height', data=val_patch_heights)
with h5.File(args.out + '/' + test_data_uri,'w') as f:
  ...
  f.create_dataset('height', data=test_patch_heights)