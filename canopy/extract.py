...
​
        ...
        pbar.update(1)
​
        # define window for chm patch 
        window= Window.from_slices((row-patch_radius,row+patch_radius+1),(col-patch_radius,col+patch_radius+1))
 
        # extract chm_patch
        chm_patch = chm_vrt.read(1, window=window)
​
​
...
​
        # append the patch and label to the lists. Also append the chm of each patch
        image_patches.append(image_patch)
        patch_heights.append(chm_patch)
...
​
    # stack the heights into a numpy array
    patch_heights = np.stack(patch_heights, axis=0)
​
    return image_patches,patch_labels, patch_heights